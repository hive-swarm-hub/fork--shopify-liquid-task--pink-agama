    ...
    def date(input, format)
      ...
      when String, Integer
        ...
        if format_cache && format_cache.key?(input)
          return format_cache[input]
        end

        date = Utils.to_date(input)
        return input unless date

        formatted = date.strftime(str_format)
        DATE_FILTER_CACHE.clear if DATE_FILTER_CACHE.size >= DATE_FILTER_CACHE_MAX_FORMATS && !DATE_FILTER_CACHE.key?(str_format)
        format_cache = (DATE_FILTER_CACHE[str_format] ||= {})
        format_cache.clear if format_cache.size >= DATE_FILTER_CACHE_MAX_ENTRIES_PER_FORMAT
        format_cache[input] = formatted
        return formatted
      end
